...
The current journal format is version 8; supported older versions are read conservatively and rewritten on the next durable update. Rolling back to an operator that cannot read the newer journal is unsupported. See [journal archives](../../contracts/journal-archives/) and [compatibility](../../reference/compatibility/). The [restart-safe lifecycle decision](../../decisions/0012-restart-safe-manual-lifecycle/) explains the design history.
