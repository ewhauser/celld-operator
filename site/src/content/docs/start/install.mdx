...
1. **Clone, build and publish the operator image.** The development `:dev` image is not a published release. Choose a unique tag and registry path. The Dockerfile cross-compiles its Go binaries for the target platform; this command publishes an image index for both common EKS node architectures. Inspect the published index and copy its top-level `sha256:…` digest for Helm.
   ...
   Authenticate Docker to your registry before pushing, and check that the inspection lists the platforms used by your EKS nodes. If the registry is private, create an image pull Secret in `celld-system` and set chart `imagePullSecrets`; see the [chart values](../../api/helm-values/). The image includes the operator and launcher binaries. The runtime itself is the separately pinned celld v0.5.0 image.
...
3. **Install the Helm chart.** The `fleetNamespaces` value creates a Role and RoleBinding in each named namespace. `fleets` must already exist. Set the operator AWS identity by creating a Pod Identity association for `celld-system/celld-celld-operator`, or attach the IRSA role annotation through a Helm values file.
   ...
   Verify or create the operator's Pod Identity association now if you chose Pod Identity. The chart's ServiceAccount is `celld-celld-operator` for this release name.
...
For a PersistentFleet, also provide `launcherImage=YOUR_REGISTRY/celld-operator@sha256:YOUR_PUBLISHED_DIGEST` and the [EBS prerequisites](../../concepts/profiles/). See [chart values](../../api/helm-values/) for other options. Never use `--local-test` on EKS; that flag is for the disposable MinIO integration harness.
