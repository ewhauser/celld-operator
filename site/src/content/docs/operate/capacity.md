...
Pause new policy actions during maintenance with `spec.maintenance.paused: true`; already issued PersistentFleet work continues recovery. Removing the policy returns toward the manual replica target and can request a blocked reduction. Check the resulting target and [scaling procedure](../scaling/) first.
