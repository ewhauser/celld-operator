...
If the CRD is missing, apply `config/crd/` from the same release as the controller and recheck. If the manager Pod is unavailable, use its Pod events and logs to distinguish image pull, configuration, and journal compatibility errors. The development manifest's `:dev` image is not a published release image; supply an image your cluster can pull. [Install](../../start/install/) has the chart procedure.
