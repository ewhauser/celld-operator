...
- `GetObject` also returns the object's ETag. A body whose ETag differs from the listed one means the writer rewrote its own record between the list and the read. Every live writer does that on every lease heartbeat, so on a link slow enough for the read to trail the listing it is the normal case, not a contradiction: the read returned one self-consistent version of the object, and the generation, epoch, lease and sample-freshness checks judge that body alone. Such a body is used and simply not remembered, so the next pass reads it again. A body whose length contradicts the size listed against a *matching* ETag is a genuine contradiction and fails the pass.
