...
func TestPreviewPoolAtomicBucketReservation(t *testing.T) {
	...
	for _, dedicated := range []bool{false, true} {
		...
		wg.Go(func() {
			if dedicated {
				f := fixture("dedicated", p.Spec.Storage.Bucket, "Bucket")
...
func TestPreviewPoolScopeIdentity(t *testing.T) {
	...
	for _, mutate := range []func(*fleet.CelldFleet){
		func(f *fleet.CelldFleet) { f.Spec.Storage.PoolRef.UID = "recreated" },
		func(f *fleet.CelldFleet) { f.Namespace = "another" },
		func(f *fleet.CelldFleet) { f.Spec.Storage.PoolRef.Name = "other" },
	...
	q = p.DeepCopy()
	q.Spec.Zone = "us-east-1b"
	if reservePreviewPool(t.Context(), c, q) == nil {
		t.Fatal("pool configuration drift accepted")
	}
	res := &fleet.CelldStorageReservation{}
	if err := c.Get(t.Context(), client.ObjectKey{Name: bucketReservationName(p.Spec.Storage.Bucket)}, res); err != nil {
		t.Fatal(err)
	}
...
func TestPreviewSharedStorageReconciliation(t *testing.T) {
	...
	dedicated := fixture("dedicated", pool.Spec.Storage.Bucket, "Bucket")
...
func TestPreviewPoolRendersSmallRuntimeAndSharedStore(t *testing.T) {
	p := previewPoolFixture(previewFixture())
	p.Spec.Storage.Endpoint = &fleet.ObjectStoreEndpoint{URL: "http://preview-store.fleets.svc:9000", CredentialsSecretName: "preview-store", Egress: fleet.CollectorEgress{Namespace: "fleets", PodLabels: map[string]string{"app": "preview-store"}}}
	f := previewFleet(previewFixture(), p)
	...
	for key, want := range map[string]string{"CELLD_BUCKET": f.Spec.Storage.URL(), "S3_ENDPOINT": p.Spec.Storage.Endpoint.URL, "AWS_ALLOW_HTTP": "true", "CELLD_MAX_RESIDENT_CELLS": "8", "CELLD_LOCAL_CACHE_MAX_BYTES": "134217728"} {
	...
	p.Spec.Scratch = &fleet.ScratchSpec{Request: "16Mi", Limit: "64Mi"}
	p.Spec.Storage.Endpoint.URL = "https://preview-store.fleets.svc:9000"
...
func TestEnvtestPreviewStorageAdmission(t *testing.T) {
	...
	pool := previewPoolFixture(p)
	pool.UID = ""
	pool.Spec.Storage.Endpoint = &fleet.ObjectStoreEndpoint{URL: "http://store:9000", CredentialsSecretName: "store", Egress: fleet.CollectorEgress{PodLabels: map[string]string{"app": "store"}}}
	pool.Spec.Scratch = &fleet.ScratchSpec{Request: "64Mi", Limit: "512Mi"}
	for _, mutate := range []func(*fleet.CelldPreviewPool){
		func(p *fleet.CelldPreviewPool) { p.Spec.Scratch.Request = "0" },
		func(p *fleet.CelldPreviewPool) { p.Spec.Scratch.Limit = "32Mi" },
		func(p *fleet.CelldPreviewPool) { p.Spec.Zone = "us-west-2a" },
		func(p *fleet.CelldPreviewPool) { p.Spec.Storage.Endpoint.URL = "http://store/other" },
	} {
	...
	for _, mutate := range []func(*fleet.CelldFleet){
		...
		func(f *fleet.CelldFleet) { f.Spec.Storage.PoolRef.UID = "other" },
	...
	for _, mutate := range []func(*fleet.CelldFleet){
		...
		func(f *fleet.CelldFleet) { f.Spec.Storage.PoolRef = nil },
...
func TestEnvtestPreviewSampleManifests(t *testing.T) {
	...
	for _, name := range []string{"preview-store.yaml", "preview-pool.yaml", "preview.yaml"} {
		...
		for {
			...
			if obj.GetKind() == "CelldPreviewPool" {
