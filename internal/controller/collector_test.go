...
func TestCollectorRuntimeAndMetrics(t *testing.T) {
	for _, scenario := range []string{"complete", "denied", "missing cpu", "stale", "future", "unknown state", "restarted", "oversize", "redirect", "bad window", "wrong identity", "unready", "canceled"} {
		t.Run(scenario, func(t *testing.T) {
			...
			r := setup(t, f, p)
			raw, err := os.ReadFile("../runtime/v050/testdata/state.json")
			if err != nil {
				t.Fatal(err)
			}
			...
			if scenario == "complete" || scenario == "unready" {
				if s.CPU != 123 || s.MemoryMiB != 100 || s.MetricsAt.IsZero() || s.RuntimeAt.IsZero() || s.Ready != (scenario == "complete") {
					t.Fatal(s)
				}
			} else {
