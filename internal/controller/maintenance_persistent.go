...
func (r *Reconciler) executePersistentMaintenance(ctx context.Context, f *fleet.CelldFleet, res *fleet.CelldStorageReservation, j *lifecycleJournal, w client.Object, block func(error) (ctrl.Result, bool, error)) (ctrl.Result, bool, error) {
	...
	view := *j
	view.Operation = &lifecycleOperation{ID: m.ID, From: j.Applied, To: j.Applied - 1, PersistentMembers: m.Persistent}
	if m.Index < len(m.Targets) {
		view.Operation.TargetPod = m.Targets[m.Index].Name
		view.Operation.TargetUID = string(m.Targets[m.Index].UID)
	}
	switch m.Phase {
	case "Capture", "Next":
		if m.Phase == "Capture" {
		...
		members, _, err := r.assessPersistent(ctx, f, &view, false, false)
		if err != nil {
			return block(err)
		}
	...
	case "Stopping":
		...
		if state.Phase == "Running" {
			...
			m.Persistent = members
			view.Operation.PersistentMembers = members
		...
		members, _, err := r.assessPersistent(ctx, f, &view, true, false)
		if err != nil {
			return block(err)
		}
		m.Persistent = members
